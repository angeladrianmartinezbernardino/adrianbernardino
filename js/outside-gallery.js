...
async function updateOrderInFirestore(ids) {
    ...
    ids.forEach((id, index) => {
        // Collection: pages/outside/outside
        const ref = doc(db, "pages", "outside", "outside", id);
...
/**
 * Loads all photos for /outside from Firestore and resolves their Storage URLs.
 * Firestore collection: pages/outside/outside
 * Required fields:
 *   album, color_label, year, title, standard_path, original_path, order
 * @returns {Promise<Array>}
 */
async function loadPhotosFromFirestore() {
    const photosRef = collection(db, "pages", "outside", "outside");
